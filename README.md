...
_**Inputs 1 - 8**_:
All of the per input channel [1-8] automatable control parameteres are listed in section 4.3.3 of the [K-Mix Manual](https://files.keithmcmillen.com/products/k-mix/documentation/kmix-manual.pdf). The input channel is denoted by ':channel-number', so for controlling input channel 3's fader level, you would use 'fader:3'.
...
To control K-Mix's miscellaneous features (reverb/surround/auxes), covered in section 4.3.3.4 in the [K-Mix Manual](https://files.keithmcmillen.com/products/k-mix/documentation/kmix-manual.pdf), send to 'misc:reverb-level', which controls the K-Mix's main reverb level
...
If you wish to control the K-Mix's 3 MIDI Banks, covered in section 4.3.4 of the [K-Mix Manual](https://files.keithmcmillen.com/products/k-mix/documentation/kmix-manual.pdf), you send to the 'k-mix-control-surface' port, referred to in this API as 'control'. The main purpose of sending to the 'control' port is for K-Mix LED control.
...
_Passing in window.performance.now() as the time would produce undesired send times, since then 'time' would equal window.performance.now() + window.performance.now()_

##Help
If you want a reference for any of the names or CC/note messages of all of K-Mix's controllable parameters call 'help' on your 'kmix' variable and pass in what you're looking for and the parameters will be to your browser's console. The 4 options are:
